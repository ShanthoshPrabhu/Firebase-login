        ...
        <div className=' flex flex-col space-y-6 p-4 w-[360px] '>
            ...
            {emailValid ? (
                <div className=' w-[100%]'>
                    ...
                    <input type="password" value={password} onChange={e => setPassword(e.target.value)} className='flex items-center justify-center  w-[100%] bg-[#f2f1ee99] outline-none focus-visible:outline-[#2383e291] rounded-sm text-md px-3 py-2 my-4' id='password' placeholder='Enter your password address here ' />
                </div>
            ) : (
                <div  className=' w-[100%]'>
            ...
            {emailValid ? (
                ...
                <div className=' flex items-center justify-center text-[#eb5757] text-md font-semibold bg-[#fdf5f2] px-3 py-2 cursor-pointer active:bg-[#ecd2c9]' 
                 onClick={()=>{setEmailValid(true)}}>Continue with email</div>
            )}
        </div>
        </div>
    )
